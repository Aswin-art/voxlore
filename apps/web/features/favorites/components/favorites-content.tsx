...
import type { FavoriteItem } from "../data/favorites-api"
...
export function FavoritesPageContent() {
  ...
  const { favorites: favoriteItems, isPending: isLoading } = useFavorites()
  ...
  const filteredList = favoriteItems
    .filter((item) => {
      const matchesSearch =
        ...
        {isLoading ? (
          ...
          <div className="p-10 text-center bg-card rounded-3xl border border-border flex flex-col items-center justify-center gap-2 mt-2">
            ...
            <span className="text-xs font-extrabold text-foreground">
              {favoriteItems.length === 0 ? "Belum Ada Favorit" : "Tidak Ada Destinasi Ditemukan"}
            </span>
            <span className="text-[11px] text-muted-foreground max-w-xs">
              {favoriteItems.length === 0
                ? "Jelajahi kebudayaan Nusantara dan simpan situs favoritmu dengan menekan tombol favorit."
