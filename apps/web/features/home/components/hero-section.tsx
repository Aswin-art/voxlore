...
function StatCard({ imageSrc, imageAlt, statNumber, title, description }: StatCardProps) {
  return (
    <div className="relative min-h-[160px] sm:min-h-[200px] lg:min-h-0 lg:flex-1 rounded-2xl sm:rounded-3xl overflow-hidden shadow-xl border border-border/40 flex flex-col justify-end p-4 sm:p-6 lg:p-8">
      ...
      <div className="relative z-10 flex flex-col items-start gap-1">
        ...
        <p className="text-[10px] sm:text-xs text-white/80 leading-relaxed hidden sm:block">
          {description}
        </p>
      </div>
    </div>
  ...
  return (
    ...
    >
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-2 sm:gap-3 w-full flex-1">
        {/* Left Side: Main Featured Culture Card */}
        ...
        {/* Right Side: Stat Cards & Explore Feature Footer */}
        <div className="lg:col-span-4 flex flex-col gap-2 sm:gap-3 h-full">
          <div className="grid grid-cols-2 lg:flex lg:flex-col gap-2 sm:gap-3 flex-[1.25]">
            <StatCard
              imageSrc="/images/borobudur-card.png"
              imageAlt="Borobudur Morning Daylight Cultural Heritage"
              statNumber="150+"
              title="Destinasi & Warisan Budaya"
              description="Dari Sabang sampai Merauke terhubung dalam panduan naratif."
            />
            <StatCard
              imageSrc="/images/dancer-card.png"
              imageAlt="Seni Tari Tradisional Legong Bali"
              statNumber="300+"
              title="Panduan Audio Sinematik"
              description="Pengalaman audio dwibahasa jernih dengan musik etnik asli."
            />
          </div>
