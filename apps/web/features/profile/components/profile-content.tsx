...
export function ProfilePageContent() {
  ...
  const userName = user?.name || "Pengguna Voxlore"
  const userEmail = user?.email || "user@voxlore.id"
  const userProfile = {
    name: userName,
    email: userEmail,
    initials: userName
    ...
    <div className="p-4 sm:p-5 flex flex-col gap-5 pb-24 relative">
      ...
      <div className="p-5 rounded-3xl bg-card text-foreground border border-border flex flex-col gap-4 shadow-2xs relative overflow-hidden">
        ...
        <div className="grid grid-cols-3 gap-2 pt-3 border-t border-border/60 relative z-10">
          ...
          {/* Card 2: Destinasi Favorit */}
          <div className="flex flex-col items-center justify-center p-2.5 rounded-2xl bg-secondary/60 border border-border text-center">
            <HugeiconsIcon icon={FavouriteIcon} className="w-4 h-4 text-emerald-500 mb-0.5" />
            <span className="text-sm font-black text-foreground">{userProfile.favoritesCount}</span>
            <span className="text-[9px] text-muted-foreground font-extrabold">Destinasi Favorit</span>
          </div>
